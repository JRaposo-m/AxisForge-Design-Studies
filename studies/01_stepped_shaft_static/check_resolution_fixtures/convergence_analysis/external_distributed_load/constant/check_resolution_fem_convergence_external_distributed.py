...
check_resolution_fem.py, with ONE addition: shaft3 gets a
DistributedRadialLoad ("bushing_process_load", source="user") on top of
its existing point sprocket_load -- the base fixture has no distributed
load anywhere, and "shaft_fem.convergence.external_distributed.*" has
nothing to converge on without one. Placed at [165.0, 185.0] mm,
deliberately clear of gear g3_driver's face width ([142.5, 157.5]) and
shaft3_brg_B's extent (~190.0 mm +/- its own small width) -- see
MeshConvergenceStudy._eval_points_for_interval()'s own containment
check, which raises if an interval only partially overlaps a bearing or
gear it doesn't fully enclose.

Exercises "shaft_fem.convergence.external_distributed.timoshenko":
runs convergence_study.run_convergence() restricted to
DistributedRadialLoad spans only (regions={"external_distributed"}),
Timoshenko theory. Only shaft3 is expected to produce a non-empty
per_load (1 interval, "bushing_process_load") -- shaft1/shaft2 have no
DistributedRadialLoad, so their MeshRefinementResult.per_load is
expected to be empty (0/0 "converged", reported honestly by
convergence_report.py's shaft_convergence_block(), not treated as a
failure).

Console output and report-writing follow the same shape as
check_resolution_fem_convergence_gears.py -- see that script's own
...
def build_system() -> tuple["ConstructionCapabilities", "SpurHelicalGearSystem"]:
    ...
    # Added specifically for this check -- uniform by default (see
    # loads.py's own DistributedRadialLoad.__init__: magnitude as a
    # plain float means q(x) = magnitude / b, constant, not linear),
    # standing in for e.g. a bushing/process load spread over a collar.
...
def main() -> None:
    ...
    out_path = HERE / "report_2stage_chain_convergence_external_distributed.txt"
    write_studies_report(
        system, out_path,
        title="2-stage linear chain -- Mesh convergence (external distributed, Timoshenko)",
