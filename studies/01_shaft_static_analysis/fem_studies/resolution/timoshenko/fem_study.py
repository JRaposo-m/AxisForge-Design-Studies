...
from axisforge.fixtures.studies.shafts.fem_studies.outputs.resolution_csv import resolution_csv
...
def main() -> None:
    ...
    libraries = {
        "hutchinson": solve_system(system, construction, shear_theory="hutchinson"),
        "cowper":     solve_system(system, construction, shear_theory="cowper"),
    }

    for shear_theory, library in libraries.items():
        expected_names = {ss.name for ss in system.shafts}
        got_names = set(library.names())
        missing = expected_names - got_names

        ok = not missing
        print(f"[{'OK' if ok else 'FAIL'}] [{shear_theory}] solved "
              f"{len(library)}/{len(system.shafts)} shafts -- {library!r}")
        if missing:
        ...
        for ss in system.shafts:
            ...
            print(f"    [{shear_theory}] {ss.name:8s} nodes={n:3d}  "
                  f"sigma_b_max={r.sigma_b_max:8.2f} MPa @ x={r.x_sigma_b_max:6.1f} mm  "
                  f"v_max={r.v_max:7.4f} mm @ x={r.x_v_max:6.1f} mm  [{brg_str}]")

        out_path = HERE / f"report_resolution_{shear_theory}.txt"
        write_studies_report(
            system, out_path,
            title=f"2-stage linear chain -- Studies report (Timoshenko, {shear_theory})",
            shaft_fem_library=library,
        )
        print(f"[OK] [{shear_theory}] {out_path.name} written ({len(system.shafts)} shafts, "
              f"from the real solve_system() library above -- no fabricated data)")

        written = write_resolution_plots(library, system, HERE, subdir=shear_theory)
        n_files = sum(len(paths) for paths in written.values())
        print(f"[OK] [{shear_theory}] {n_files} plot(s) written -- "
            f"for {len(written)}/{len(system.shafts)} shaft(s) "
            f"-- from the same library above, no second solve")
        # ver nota abaixo sobre a assinatura de write_resolution_plots
        n_files = sum(len(paths) for paths in written.values())
        print(f"[OK] [{shear_theory}] {n_files} plot(s) written -- "
              f"for {len(written)}/{len(system.shafts)} shaft(s) "
              f"-- from the same library above, no second solve")

        csv_dir = HERE / "csv" / shear_theory
        ...
        for ss in system.shafts:
            ...
            if r is None:
                continue
            csv_path = csv_dir / f"{ss.name}_resolution.csv"
            csv_path.write_text(resolution_csv(r, decimals=10), encoding="utf-8")
            n_csv += 1
        print(f"[OK] [{shear_theory}] {n_csv} CSV(s) written under "
              f"{csv_dir.relative_to(HERE)}/ -- from the same library above, no second solve")
